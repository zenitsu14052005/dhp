  ...
  run: async ({ client, message, args, ServerData, Color }) => {
    ...
    if (!query) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `You Din't Provide A Query Or Song Link. Please contiue with ${prefix}spotify <query/url>`
        );
    ...
    if (
      ...
      try {
        ...
        if (!result.tracks.length) return message.reply("\`\`\`diff\n-No results found!\`\`\`");
        ...
        if (result.type === "PLAYLIST") {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `Added To Queue [${result.tracks[0].title}](${query2}) by ${result.tracks[0].author}`
            );
          return msg.edit({ embeds: [embed] });
        }
      } catch (err) {
