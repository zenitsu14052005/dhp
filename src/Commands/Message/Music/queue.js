...
export default {
  ...
  run: async ({ client, message, emojis, args }) => {
    ...
    const ColdtotalSongsPerPage = 10;
    const totalPages = Math.ceil(player.queue.length / ColdtotalSongsPerPage);
    let page = 1;
    if (args[0]) {
      if (isNaN(args[0])) {
        const embed = new EmbedBuilder()
    ...
    const start = (page - 1) * ColdtotalSongsPerPage;
    const end = page * ColdtotalSongsPerPage;
